...
public class _03_크레인_인형뽑기_카카오 {
    ...
    public static void main(String[] args){
        _03_크레인_인형뽑기_카카오 T = new _03_크레인_인형뽑기_카카오();
    ...
    private int solution(final int[][] board, final int[] moves) {
        ...
        for(int pos :  moves){
            for (int i = 0; i < board.length; i++){
                if(board[i][pos - 1] != 0){
                    int caughtDoll = board[i][pos -1];
                    board[i][pos - 1] = 0;
                    if(!stack.isEmpty() && caughtDoll == stack.peek()){
                        answer += 2;
                        stack.pop();
                    }else stack.push(caughtDoll);
                    break;
                }
            }
        }
